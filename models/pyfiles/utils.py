...
class Data_utility(object):
    ...
    def __init__(self, args):
        ...
        ## skip creation of validation and test sets, use all available data for training
    ...
    def test_batchify(self, test_dat):

        
        full_dat = np.append(self.dat, test_dat, axis=0)
        idx_set = range(self.n, self.n+np.shape(test_dat)[0])
        n = len(idx_set)
        X = torch.zeros((n, self.P, self.m))
        Y = torch.zeros((n, self.m))
        ...
        for i in range(n):
            ...
            start = end - self.P
            X[i,:self.P,:] = torch.from_numpy(full_dat[start:end, :])
            Y[i,:] = torch.from_numpy(full_dat[idx_set[i], :])
